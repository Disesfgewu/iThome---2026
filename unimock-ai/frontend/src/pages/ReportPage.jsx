...
`;
  ...
  const questionDiagnoses = (report.question_diagnoses && report.question_diagnoses.length > 0)
    ...
    : (dialogueHistory.length > 0 ? dialogueHistory.map((item, idx) => {
        ...
        const isMed = ['醫學', '中醫', '牙醫', '護理', '藥學', '臨床', '醫藥'].some(k => targetMajor.includes(k));
        ...
        if (isMed) {
          if (turnNum === 1) {
            weakness = `自我介紹表達真誠，建議加強說明臨床同理心、醫學倫理思維，以及選擇報考 ${targetMajor} 的核心動機。`;
            improved = `教授您好，我是報考${targetMajor}的考生。在修習生物與化學專業基礎的過程中，我曾於醫療機構擔任志工，近距離觀察臨床醫師在面對急診同理陪伴與醫療決策時的溝通條理。在專題實驗中，我透過嚴謹的數據分析與反覆驗證克服實驗瓶頸，體會到醫學研究的科學精神。我期許自己能在貴系紮實的臨床與前瞻醫療資源中成長，成為兼具精準醫療專業與人文關懷的臨床醫師。`;
          } else if (turnNum === 2) {
            weakness = '情境應變答覆得體，若能進一步補充具體溝通步驟與醫病權衡（Empathy & Decision Trade-off）細節，說服力將更加卓越。';
            improved = `教授您好，當面對醫療決策衝突或高壓臨床情境時，我認為首要原則是保持冷靜並建立雙向溝通。在過往團隊協作與志工經驗中，我曾遇到成員意見分歧的情形，當時我先傾聽各方關切點，並透過客觀數據與結構化分析提出調和方案，最終順利化解爭議。這段經歷讓我體會到，優秀的醫師不僅需要紮實的醫學知識，更需要強大的抗壓韌性與傾聽同理能力。`;
          } else {
            weakness = `前瞻視角豐富，若能連結智慧醫療 AI 輔助診斷與 ${targetMajor} 特色研究領域，講述深度將更臻完善。`;
            improved = `教授您好，對於醫學領域的前瞻發展，我特別關注智慧醫療 AI 輔助診斷與基因個體化治療的結合。我希望在進入${targetMajor}後，除修習臨床基礎醫學，也能積極參與前瞻醫學研究中心之專案，探究如何利用大數據提升早期病灶篩檢精準度，同時維護醫療倫理與病患隱私，為未來臨床智慧醫療貢獻心力。`;
          }
        } else if (isBiz) {
          if (turnNum === 1) {
            weakness = `自我介紹表達沉著，建議加強說明高階管理視角、資產與風險管理決策，以及報考 ${targetMajor} 的核心動機。`;
            improved = `教授您好，我目前任職於金融機構擔任風控主管，長期負責跨國資產負債管理與法遵決策。在實務工作中，我發現傳統的風控框架在面對利率快速波動與供應鏈碎片化的環境時，往往難以即時反應，因此我主動導入敏感度分析模型與結構化避險機制，有效控制了資金成本的上升幅度，維護了公司財務結構的穩健。這些實戰積累讓我深刻體認到系統性管理知識的重要性，這也是我報考${targetMajor}、希望與教授們深入探討高階戰略決策的核心動機。`;
          } else if (turnNum === 2) {
            weakness = '實務經驗述說清晰，但建議進一步補充具體管理決策架構與量化成效指標。';
            improved = `教授您好，在面對美聯儲持續升息以及國際供應鏈加速重組的背景下，我在公司主持了一項外匯避險與流動性壓力測試的整合性專案。我們建立了動態敏感度分析模型，並導入風險權重監控流程，每週對資金部位進行滾動式評估，確保在極端情境下仍能維持足夠的流動性緩衝。最終，我們將整體資金成本的增幅控制在董事會核准的目標範圍以內，也讓我在${targetMajor}的學習上，對於跨境金融決策架構有了更強的探索動力。`;
          } else {
            weakness = `專業觀點極具前瞻性，若能深化 ESG 綠色金融與 AI 自動化審查之落地戰略，說服力將更加卓越。`;
            improved = `教授您好，在思考未來的研究方向與職涯規劃時，我對於綠色金融與 ESG 永續放款標準的整合應用特別感興趣。目前市場上許多機構在推動永續金融時，仍面臨資料標準不一與審查流程繁瑣的挑戰，我認為可以透過引入 AI 自動化風控審查來提升效率，同時兼顧法遵與隱私規範。這也是我在${targetMajor}最希望深入研究的議題，期望能將學術框架與實務場景結合，為企業的數位與永續轉型貢獻具體方案。`;
          }
        } else {
          if (turnNum === 1) {
            weakness = `自我介紹條理尚屬清晰，但建議加強『報考 ${targetMajor} 的核心動機』與『具體專案成果/競賽數據』的連結。`;
            improved = `教授您好，我在修習專業基礎課程與推動相關專案的過程中，逐步建立了對${targetMajor}核心議題的強烈興趣。其中一次令我印象最深刻的經驗，是在模組化設計的專案中遭遇效能瓶頸，我透過系統性的結構化測試與反覆優化，最終將執行效能提升了約 35%，也獲得了指導教授的肯定。這段解題歷程讓我深刻體會到，唯有具備紮實的理論基礎，才能在面對複雜問題時做出有根據的決策，這正是我選擇報考${targetMajor}、期望在更嚴謹的學術環境中繼續成長的主要動因。`;
          } else if (turnNum === 2) {
            weakness = '技術細節回答明確，但建議補充『演算法/架構 Trade-off 選擇考量』與『最終量化效能指標』。';
            improved = `教授您好，在專案開發中我遇到了核心模組的關鍵效能瓶頸，當時系統在高並發查詢下，推論精準度與回應速度之間存在明顯的取捨。我先做了多種方案的對比分析，最終決定採用混合式架構，在過濾演算法層加入數據快取緩衝機制，成功將平均回應延遲降低至毫秒等級，同時系統吞吐量也顯著提升。這段經歷讓我認識到，工程決策不能只追求單一指標的最優，必須在多重約束下找到最合適的平衡，這也是我希望在${targetMajor}繼續深化的研究方向。`;
          } else {
            weakness = `回答具備良好說服力，若能進一步連結 ${targetMajor} 最新前瞻趨勢（如 AI 結合企業流程與資安防護），講述深度將更臻完善。`;
            improved = `教授您好，對於${targetMajor}領域的前瞻應用，我近期特別關注大型語言模型如何與企業的核心業務流程做深度整合。在評估導入可行性時，我發現資訊安全規範與模型可解釋性是最關鍵的兩道門檻，因此我嘗試提出一套兼顧隱私合規與效能優化的架構設計，並在小規模的驗證環境中取得了正向的初步成果。我希望能在${targetMajor}的學習過程中，與教授們進一步探討這個方向的理論基礎與落地策略。`;
          }
        }
      ...
      }) : [
        {
          ...
          improved_sample: "教授您好，在修習專業基礎與推動相關專案的過程中，我對目標學系的核心議題有了更深的體認。我在一次模組化設計專案中遭遇效能瓶頸，透過系統性測試與多輪優化，最終達成了顯著的效能提升。這段解題歷程讓我確信，進入貴所深化理論基礎，是我下一個成長階段最重要的決定。"
        }
      ]);
