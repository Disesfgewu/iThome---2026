...
`;
  ...
  const questionDiagnoses = (report.question_diagnoses && report.question_diagnoses.length > 0)
    ...
    : (dialogueHistory.length > 0 ? dialogueHistory.map((item, idx) => {
        ...
        let weakness = `回答表達流暢，建議進一步深化與 ${targetMajor} 核心專業理念的連結，並補充具體專案或實務成果。`;
        let improved = '';

        if (turnNum === 1) {
          weakness = `自我介紹條理尚屬清晰，但建議加強說明報考 ${targetMajor} 的核心動機與具體實務/競賽成果連結。`;
          improved = `教授您好，我是報考${targetMajor}的考生。在修習相關基礎課程與推動專案的過程中，我逐步建立了對${targetMajor}核心議題的強烈興趣。在一次專案實作中，我透過系統性的架構設計與優化，成功提升了整體成效。這段歷程確立了我深入貴系學習、培養專業實力與解決複雜問題能力的堅定決心。`;
        } else if (turnNum === 2) {
          weakness = `技術與實作細節回答明確，若能補充在遭遇挑戰時的架構決策考量與最終量化指標，說服力將更加卓越。`;
          improved = `教授您好，在專案開發時我曾遭遇關鍵瓶頸與多重約束。在權衡不同方案後，我主動導入結構化分析與優化機制，成功克服技術障礙並大幅提升系統效能。這段經驗讓我深刻體認到，優秀的專業人才必須在實務與理論之間找到最佳平衡，這也是我想在${targetMajor}進一步深造的主因。`;
        } else {
          weakness = `觀點具備良好潛力，若能精準連結 ${targetMajor} 未來修課藍圖與前瞻發展方向，論述深度將更臻完善。`;
          improved = `教授您好，關於未來的學習與研究規劃，我希望在進入${targetMajor}後，除了紮實掌握核心理論與實作技能，也能參與前瞻專案研究，結合跨領域知識解決實際問題，期許未來能成為具備架構思維與團隊領導力的專業人才。`;
        }
