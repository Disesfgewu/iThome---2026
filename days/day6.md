...
#### 4. 資料庫去識別化解耦與後端動態組合架構 (Runtime Dynamic Assembly Architecture)

**核心設計原則：資料庫題庫層與學校階層完全解耦 (Decoupled)!**
資料庫內部的每筆題目僅保存客觀的技術屬性（學群、學系、問題類別、難易度），**絕不硬編碼或綁定任何具體學校**。

當學生發起模擬面試時，由後端服務在 **Runtime (運行時)** 結合前端輸入的「目標學校」與「目標學系」，動態計算出該場面試的 **「難易度配比矩陣 (Difficulty Ratio)」** 與 **「問題屬性配比矩陣 (Category Ratio)」**，再向 `QuestionRepository` 抽取與組合題目：

```mermaid
graph TD
    A["前端輸入: 目標學校 + 目標學系"] --> B["後端 Runtime 組合引擎 (checkSchoolTier)"]
    B -->|動態計算| C["難易度配比矩陣 (高難度50% / 進階30% / 標準20%)"]
    B -->|動態計算| D["問題屬性配比矩陣 (技術專業50% / 通用20% / 英文20% / 申論10%)"]
    C & D --> E["QuestionRepository (多維度安全過濾與向量抽取)"]
    E --> F["產生客製化考官面試題庫組合"]
```

| 目標學校階層 (Target School Tier) | 後端動態難易度配比 (Difficulty Ratio) | 後端動態問題屬性配比 (Category Ratio) |
| :--- | :--- | :--- |
| **頂尖研究型名校**<br>(如台大、清大、交大、成大、政大等) | 高難度申論題 **50%**<br>進階專業題 **30%**<br>標準題 **20%** | 技術專業型問題 **50%**<br>情境申論型問題 **30%**<br>通用型 / 英文題 **20%** |
| **前段國立與特色熱門學系**<br>(如中央、中興、中正、中山等) | 進階專業題 **50%**<br>標準題 **30%**<br>高難度申論題 **20%** | 技術專業型問題 **40%**<br>通用型問題 **40%**<br>英文型 / 情境申論題 **20%** |
| **通用大專院校 / 地區國立 / 私立名校** | 標準題 **60%**<br>進階專業題 **30%**<br>高難度申論題 **10%** | 通用型問題 **60%**<br>技術專業型問題 **30%**<br>英文型 / 情境題 **10%** |

#### 5. RAG 知識檢索與 LLM 動態題目生成架構 (RAG + LLM Dynamic Question Generation)
...
1. **RAG 脈絡種子 (Context Seeds)**：
   `QuestionRepository` 根據「難易度配比」與「問題類別配比」進行向量相似度與多維度過濾，抽取出相關度最高的題庫種子、 Rubric 評分規準與參考擬答。
...
1. **跨系通用題庫 (`通用大專院校` / `通用學系`)**：
   - 包含約 300+ 筆適用於**任何學系**的基礎面試題目（如面試禮儀、個人優缺點、挫折應变、讀書計畫）。
