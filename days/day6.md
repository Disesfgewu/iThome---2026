...
#### 4. 資料庫欄位用途說明與去識別化解耦架構

**Day 6 核心任務在於「資料庫的結構化建構」與「欄位標籤設計」！**
資料庫內部的每筆題目僅保存客觀的技術屬性與分類標籤（學群、學系、問題類別、難易度），**絕不安插或硬編碼任何具體學校**。

```mermaid
graph TD
    A["面試題庫資料庫 (interview_questions_db.json)"] --> B["客觀分類標籤"]
    B --> C["學群 / 學系標籤 (department_group / department)"]
    B --> D["題型屬性標籤 (question_category: 通用/技術專業/英文/申論)"]
    B --> E["難易度分級標籤 (difficulty_level: 標準/進階/高難度)"]
    B --> F["3072 維度語意向量 (embedding)"]
```

#### 5. 各分類標籤於後端運作之用途 (Field Roles in RAG Engine)

- **`department_group`（學群標籤用途）**：
  供後端進行跨學系的大領域配對與通用問題推薦（例如：將資訊工程、電機工程統一歸類於「資訊電機學群」）。
- **`department`（學系標籤用途）**：
  供後端精確過濾學生目標申請的特定學系專屬題目。
- **`question_category`（題型屬性標籤用途）**：
  供後端決定該場面試的題型組合（如：混搭個人選系動機的「通用型」、核心知識的「技術專業型」、雙語面試的「英文型」或倫理思考的「情境申論型」）。
