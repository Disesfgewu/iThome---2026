...
- **通用型問題**：聚焦於個人特質、選系動機、學習歷程檔案、團隊合作經驗與軟實力展現。
- **技術專業型問題**：聚焦於學科核心知識、專業術語解釋、技術原理、程式演算法與邏輯推理能力。
- **情境申論型問題**：聚焦於專業道德倫理困境、時事應變、跨領域整合與開放式高難度申論題。

#### 4. 難易度分級維度 (`difficulty_level`)

- **標準題**：基礎觀念、一般性選系動機與常規面試問題（適用於所有大專院校面試）。
- **進階專業題**：核心學科進階題、實作題與技術延伸（適用於前段國立與特色學系）。
- **高難度申論題**：頂尖研究型大學（如台清交成政）之專業申論大題、研究方法與綜合情境應變。

#### 5. 評分規準四維度 (`rubric`)

- `logic_structure`：評估學生是否採用 STAR 原則且條理清晰。
- `major_relevance`：評估專業術語使用正確度與目標學系契合度。
- `communication_clarity`：評估表達表達流暢度、自信心與語速。
