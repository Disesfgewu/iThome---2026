...
依據升學面試實務需求，我們進一步對 **[SetupPage.jsx](file:///c:/Users/marti/Desktop/iThome---2026/unimock-ai/frontend/src/pages/SetupPage.jsx)** 的參數輸入機制進行了彈性重構：

### A. 參數輸入元件設計
1. **目標學校 (Target School - 文字輸入)：**  
   允許學生自由輸入欲申請之目標大專院校（如：`國立臺灣大學`、`國立成功大學`、`國立陽明交通大學`）。
2. **目標學群 (Department Group - 下拉選單)：**  
   提供國內大專院校 10 大學群選單（`資訊電機學群`、`醫藥衛生學群`、`數理化學學群`、`工程學群`、`管理學群`、`財經學群`、`外語學群`、`人文社會學群`、`生物資源學群`、`建築與設計學群`）。
3. **目標學系 (Target Major - 文字輸入)：**  
   支援完全自訂文字輸入，因應跨領域學程與新設學系（如：`資訊工程學系`、`智慧醫療與健康資料學程`），並提供熱門學系快速點擊填入。

### B. 學校難度分級與 AI Persona 出題連動 (School Tier Scaling)
- **頂尖/前段名校模式：**  
  當 AI Persona 偵測到目標學校為頂尖前段大學（如：台大、清大、交大、成大、政大等）時，系統將自動啟動 **「高難度專業技術與申論題型 Mode」**，評測引擎會出具較高難度的技術底層原理、演算法比較與長篇開放式申論大題。
...
[1. 面試設定 (Setup)] ──► [2. 實戰面試艙 (Interview)] ──► [3. 評測報告 (Report)] ──► [4. 歷次練習 (History)]
   - 學校/學系文字輸入       - 打字機發問動畫           - HTML5 Canvas 雷達圖     - 成績視覺進度條
   - 10 大學群下拉選單        - 16 軌音波動畫            - STAR 對照折疊卡片       - 學校/學群搜尋
   - 名校難度動態標籤         - 名校難度發問 Mode        - 完整學校與學系報告       - 狀態標籤過濾
```
